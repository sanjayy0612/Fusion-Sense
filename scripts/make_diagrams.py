...
def architecture():
    ...
    box(ax, 0.2, 0.8, 1.9, 0.7, "OV2640\nCamera", GREEN, 9)
    box(ax, 2.9, 2.6, 1.9, 0.7, "Wearable ESP32\nIMU gateway", ORANGE, 9)
    box(ax, 2.9, 0.8, 1.9, 0.7, "ESP32-CAM\nMJPEG stream", ORANGE, 9)
    box(ax, 5.5, 0.6, 2.4, 2.7, "Laptop API\n\npose extraction\nsync + windowing\nFusionWindow", BLUE, 9)
    box(ax, 8.5, 1.9, 3.2, 1.2, "FusionSense V1\ncamera+IMU fusion\nradar masked invalid", BLUE, 9)
    box(ax, 8.5, 0.5, 3.2, 0.9, "API/MQTT -> Dashboard\nactivity + trust + fall alert", GREY, 9)
    arrow(ax, 2.1, 2.95, 2.9, 2.95); arrow(ax, 4.8, 2.95, 5.5, 2.6)
    arrow(ax, 2.1, 1.15, 2.9, 1.15); arrow(ax, 4.8, 1.15, 5.5, 1.3)
...
def physical():
    ...
    box(ax, 0.7, 4.3, 2.4, 0.7, "ESP32-CAM + OV2640\n(on wall/tripod)", GREEN, 9)
    ...
    arrow(ax, 3.1, 4.2, 6.0, 2.9); ax.text(4.3, 3.9, "camera observes\nperson", fontsize=8, color=GREY)
...
def circuit():
    ...
    box(ax, 0.4, 0.7, 2.3, 1.0, "OV2640 + ESP32-CAM\n(separate video node)", GREEN, 9)
    wire(2.7, 1.2, 8.2, 2.5, BLUE, "Wi-Fi MJPEG -> OpenCV")
    ax.set_title("FusionSense V1 — Hardware Connections", fontsize=13, weight="bold")
